...
def parse_html_document(url: str, html: str) -> ParsedDocument:
    # Crucial Fix: Only parse the actual documentation article if it exists, dropping the massive global sidebar
    article_match = _ARTICLE_RE.search(html)
    if article_match:
        html = article_match.group(1)

    # Remove JSON/JS payloads AND local navigation sidebars/footers before parsing text
    prev = ""
    while html != prev:
        prev = html
